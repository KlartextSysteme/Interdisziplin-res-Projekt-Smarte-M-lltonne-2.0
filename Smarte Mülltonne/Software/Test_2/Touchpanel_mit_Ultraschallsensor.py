...
class UltraschallSensor:
    ...
    def fuellstand_prozent(self):
        distanz = self.messen_cm()

        if distanz is None:
            return None

        # Anpassen an eure Mülltonne
        DISTANZ_LEER = 40   # cm: Abstand Sensor bis Boden
        DISTANZ_VOLL = 8    # cm: Abstand Sensor bis Mülloberfläche bei voll

        if distanz > DISTANZ_LEER:
            distanz = DISTANZ_LEER

        if distanz < DISTANZ_VOLL:
            distanz = DISTANZ_VOLL

        fuellstand = (DISTANZ_LEER - distanz) / (DISTANZ_LEER - DISTANZ_VOLL) * 100

        return int(fuellstand)
...
ui = TouchUi(display)
ultraschall = UltraschallSensor(trig_pin=0, echo_pin=1)
...
while True:
    ...
    if zaehler >= 20:
        ...
        fuellstand = ultraschall.fuellstand_prozent()

        if fuellstand is not None:
            ui.fill_level = fuellstand
